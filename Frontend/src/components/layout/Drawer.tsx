...
const Drawer = ({ info, drawerInfoFilter, open , onClose, onElementClicked }: { info: DrawerInfo, drawerInfoFilter: Record<string, string>, open: boolean, onClose: () => void, onElementClicked: (id: any) => void }) => {           
    ...
    if (!open) return null
    return(
        <div className="pointer-events-none fixed inset-0" aria-labelledby="slide-over-title" role="dialog" aria-modal="true">
            <div className="absolute inset-0 overflow-hidden">
                ...
                <div className="fixed inset-y-0 right-0 pl-10 max-w-full flex">
                    <div className="shadow-xl relative w-screen max-w-md">
                        <div className="pointer-events-auto h-full flex flex-col py-6 bg-white shadow-xl">
                            ...
                            <div className="absolute inset-0 px-4 sm:px-6">
                                <div className="flex flex-col items-start justify-start gap-3 h-full border-2 border-dashed border-gray-200 overflow-hidden overflow-y-scroll" aria-hidden="true">
                                    ...
                                    {info.objects.map((obj, i) => (
                                        <div key={i} onClick={() => handleAssetClicked(i)} className="w-full shadow-md mb-4 p-3 border rounded bg-white cursor-pointer hover:bg-gray-100" >
                                            {Object.entries(obj)
                                                ...
                                                .map(([key, value]) =>
                                                <div key={key} className="flex justify-stretch py-1" >
