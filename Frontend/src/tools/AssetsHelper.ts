...
export const assetsByUser = async (userId: string) => {
    const url = new URL(`../Asset/${userId}`, import.meta.env.VITE_BACKEND_BASE_URL_HTTPS).href
    const response = await fetch(url, {
        method: 'GET',
        credentials: 'include'
    })
...
export const assetPatch = async (assetId: number, data: Record<string, string>) => {
    const url = new URL(`../Asset?id=${assetId}`, import.meta.env.VITE_BACKEND_BASE_URL_HTTPS).href
    const response = await fetch(url, {
        method: 'PATCH',
        credentials: 'include',
        headers: {
